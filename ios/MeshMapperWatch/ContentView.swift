...
/// Phase 2 debug dump.
///
/// Deliberately ugly: this exists to prove the transport carries real state
/// and that commands round-trip with an ack. Phase 3 replaces it with the map.
struct ContentView: View {
  ...
  var body: some View {
    ScrollView {
      ...
      // The whole dump dims when the phone has gone quiet, so stale data is
  ...
  private func session(_ s: WatchSnapshot) -> some View {
    VStack(alignment: .leading, spacing: 2) {
      ...
      HStack(spacing: 4) {
        ...
        // Absolute deadline rendered locally — no per-second traffic.
