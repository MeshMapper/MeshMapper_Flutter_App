...
import 'external_surface_geo_models.dart';
...
class ExternalSurfaceGeoBuilder {
  ...
  static ExternalSurfaceColor pingColor(String kind, bool success) {
    switch (kind) {
      case 'tx':
        return ExternalSurfaceColor.fromColor(
          success ? PingColors.txSuccess : PingColors.txFail,
        );
      case 'rx':
        return ExternalSurfaceColor.fromColor(PingColors.rx);
      case 'disc':
        return ExternalSurfaceColor.fromColor(
      ...
      default:
        return ExternalSurfaceColor.fromColor(PingColors.noResponse);
    }
  }

  static ExternalSurfaceColor _txPingColor(TxPing ping) {
  ...
  static ExternalSurfaceColor? latestPingColor({
    ...
    DateTime? latestAt;
    ExternalSurfaceColor? latestColor;

    void consider(DateTime at, ExternalSurfaceColor color) {
  ...
  /// Most recent pings, newest first, capped at [ExternalSurfaceGeoWire.maxPings].
  ///
  ...
  static List<ExternalSurfacePing> buildPings({
    required List<TxPing> txPings,
    required List<RxPing> rxPings,
    required List<DiscLogEntry> discLogEntries,
    required List<TraceLogEntry> traceLogEntries,
    int cap = ExternalSurfaceGeoWire.maxPings,
  }) {
    final pings = <ExternalSurfacePing>[];
  ...
  static List<ExternalSurfaceRepeater> buildRepeaters({
    ...
    if (lat != null && lon != null) {
      ...
      final ranked = located
          .map((r) => (
                repeater: r,
                distance: ExternalSurfaceGeoWire.distanceMeters(
                    lat, lon, r.lat, r.lon),
              ))
          .toList()
  ...
  static ExternalSurfaceColor overlayTypeColor(OverlayPingType type) =>
      switch (type) {
        OverlayPingType.tx =>
          ExternalSurfaceColor.fromColor(PingColors.txSuccess),
        OverlayPingType.disc =>
          ExternalSurfaceColor.fromColor(PingColors.discSuccess),
        OverlayPingType.trace =>
          ExternalSurfaceColor.fromColor(PingColors.traceSuccess),
        OverlayPingType.rx => ExternalSurfaceColor.fromColor(PingColors.rx),
      };

  /// SNR quality colour shared by every native glance surface.
  static ExternalSurfaceColor snrColor(double snr) =>
      ExternalSurfaceColor.fromColor(PingColors.snrColor(snr));
  ...
  static List<ExternalSurfaceHeardNode> buildHeard({
    ...
    final rows = <ExternalSurfaceHeardNode>[];
    ...
    return rows.length > ExternalSurfaceGeoWire.maxHeard
        ? rows.sublist(0, ExternalSurfaceGeoWire.maxHeard)
        : rows;
  }

  static ExternalSurfaceHeardNode _row({
    required String id,
    required double snr,
    ...
    if (lat != null &&
      ...
      distance = ExternalSurfaceGeoWire.distanceMeters(
          lat, lon, repeater.lat, repeater.lon);
    }

    return ExternalSurfaceHeardNode(
      id: hex,
