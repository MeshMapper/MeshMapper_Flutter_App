...
class CryptoService {
  ...
  static Uint8List deriveChannelKey(String channelName) {
    ...
    if (!channelName.startsWith('#')) {
      throw FormatException('Channel name must start with # (got: "$channelName")');
    }
    ...
    debugLog('[CRYPTO] Channel key derived successfully (${channelKey.length} bytes)');
  ...
  static Uint8List decryptChannelMessage(
    ...
    if (channelKey.length != 16) {
      throw ArgumentError('Channel key must be 16 bytes (got ${channelKey.length})');
    }
  ...
  static String? parseChannelMessage(
    ...
    try {
      ...
      if (printableRatio > 0.8) {
        ...
        debugWarn('[CRYPTO] Message not printable (${(printableRatio * 100).toStringAsFixed(1)}% printable)');
        return null;
      }
    } catch (e) {
