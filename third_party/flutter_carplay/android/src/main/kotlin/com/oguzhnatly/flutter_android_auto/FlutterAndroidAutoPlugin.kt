...
class FlutterAndroidAutoPlugin : FlutterPlugin, EventChannel.StreamHandler {
    ...
    private suspend fun buildTemplateForType(
        ...
        // DELTA D: see FAASurfaceProvider in AndroidAutoService.kt.
        "FAAMapWithContentTemplate" ->
            getMapWithContentTemplate(data, addBackButton, owningScreen)
    ...
    /// DELTA D from upstream 1.6.5: MapWithContentTemplate support.
    ///
    /// The host draws nothing here — the app does, onto a Surface it gets from
    /// AppManager.setSurfaceCallback. All this builds is the frame: a map region
    /// the app renders into, plus whatever content template Dart nested inside
    /// it, which goes through the same dispatch so any supported template works
    /// as the content half.
    ///
    /// Requires car API level 7 and the androidx.car.app.MAP_TEMPLATES and
    /// androidx.car.app.ACCESS_SURFACE permissions, and an app category that is
    /// permitted to draw maps (navigation, POI, or weather — not IOT).
    private suspend fun getMapWithContentTemplate(
        data: Map<String, Any?>,
        addBackButton: Boolean,
        owningScreen: Screen?,
    ): Template {
        val contentType = data["contentRuntimeType"] as? String
            ?: throw IllegalArgumentException("contentRuntimeType is required")
        val contentData = data["contentTemplate"] as? Map<String, Any?>
            ?: throw IllegalArgumentException("contentTemplate is required")

        val content = buildTemplateForType(contentType, contentData, addBackButton, owningScreen)
        val builder = MapWithContentTemplate.Builder().setContentTemplate(content)

        @Suppress("UNCHECKED_CAST")
        val actions = (data["mapActions"] as? List<Map<String, Any?>>).orEmpty()
        if (actions.isNotEmpty()) {
            val strip = ActionStrip.Builder()
            for (action in actions) {
                strip.addAction(createMapAction(currentScreen?.carContext, action))
            }
            builder.setMapController(
                MapController.Builder().setMapActionStrip(strip.build()).build(),
            )
        }
        return builder.build()
    }

    /// An action for the vertical strip the host draws down the right edge of
    /// the map.
    ///
    /// Deliberately icon-only: ACTIONS_CONSTRAINTS_MAP leaves maxCustomTitles at
    /// zero, so setting a title here gets the whole template rejected. The title
    /// still travels from Dart and is used only as the content description.
    private suspend fun createMapAction(
        carContext: CarContext?,
        action: Map<String, Any?>,
    ): Action {
        val elementId = action["_elementId"] as? String ?: ""
        val imageUrl = action["imageUrl"] as? String
        val builder = Action.Builder()

        val icon = if (carContext != null && imageUrl != null) {
            resolveCarIcon(carContext, null, imageUrl, null)
        } else null
        if (icon != null) builder.setIcon(icon)

        if (action["isPrimary"] == true) builder.setFlags(Action.FLAG_PRIMARY)
        if (action["onPress"] == true) {
            builder.setOnClickListener {
                sendEvent(
                    type = FAAChannelTypes.onMapActionPressed.name,
                    data = mapOf("elementId" to elementId),
                )
            }
        }
    ...
    private suspend fun getPaneTemplate(
        ...
        // DELTA D: an empty title means no header. build() does not require one,
        // and setting an empty CarText would draw an empty header bar rather
        // than none at all.
        val paneTemplateBuilder = PaneTemplate.Builder(paneBuilder.build())
        if (template.title.isNotEmpty()) paneTemplateBuilder.setTitle(template.title)
