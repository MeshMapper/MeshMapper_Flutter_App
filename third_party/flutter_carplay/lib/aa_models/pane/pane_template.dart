...
class AAPaneTemplate implements AATemplate {
  ...
  /// DELTA D from upstream 1.6.5: optional, and empty by default.
  ///
  /// Upstream requires a non-empty title, but `PaneTemplate.Builder.build()`
  /// does not — it validates only the pane's rows and actions. The title is what
  /// draws the header, so an empty one means no header, which matters when the
  /// pane is the content half of a MapWithContentTemplate and every row of
  /// chrome is taken from the map.
  final String title;
