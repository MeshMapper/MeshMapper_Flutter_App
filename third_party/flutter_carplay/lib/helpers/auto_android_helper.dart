...
class FlutterAutoAndroidHelper {
  ...
  /// DELTA D: the map action strip's actions live on the map template itself,
  /// not on the content template nested inside it, so the pane lookup above
  /// cannot find them.
  AAMapAction? findAAMapAction({
    required List<AATemplate> templates,
    required String elementId,
  }) {
    for (final template in templates) {
      if (template is AAMapWithContentTemplate) {
        for (final action in template.mapActions) {
          if (action.uniqueId == elementId) return action;
        }
      }
    }
